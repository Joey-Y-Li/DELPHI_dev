...
from predict import log_time, get_array_of_float_from_a_line, Read1DFeature, Split1Dlist2NpArrays, \
...
def GetProgramArguments():
    ...
    optional.add_argument('-unit', '--lstm_unit', type=int, default=64, help='int: number of units in LSTM model')
    optional.add_argument('-ks', '--kernel_size', type=int, default=5, help='int: kernel size in convolution')
    optional.add_argument('-fil', '--filter_size', type=int, default=48, help='int: filter size in convolution')
...
def LoadLabelsAndFormatFeatures(args):
    ...
    assert (
            ECO_3D_np.shape == RAA_3D_np.shape == RSA_3D_np.shape == Anchor_3D_np.shape == HYD_3D_np.shape == PKA_3D_np.shape == PHY_Char_3D_np_1.shape == PHY_Char_3D_np_2.shape == PHY_Char_3D_np_3.shape == PHY_Prop_3D_np_1.shape == PHY_Prop_3D_np_2.shape == PHY_Prop_3D_np_3.shape == PHY_Prop_3D_np_4.shape == PHY_Prop_3D_np_5.shape == PHY_Prop_3D_np_6.shape == PHY_Prop_3D_np_7.shape == Pro2Vec_1D_3D_np.shape == HSP_3D_np.shape == PSSM_3D_np_20.shape == PSSM_3D_np_19.shape == PSSM_3D_np_1.shape == POSITION_3D_np.shape)

    all_features_3D_np = np.concatenate(
        (ECO_3D_np, RAA_3D_np, RSA_3D_np, Pro2Vec_1D_3D_np, Anchor_3D_np, HSP_3D_np, HYD_3D_np, PKA_3D_np,
...
def BuildModel(args):
    ...
    # the ensemble model
        ...
        model = Model(inputs=input_features, outputs=out)
        model.load_weights(
            "/home/j00492398/test_joey/interface-pred/Src/savedModel/bestModels/CNN_best_model_and_weights.h5",
            by_name=True)
        logging.info("loading weight RNN")
        model.load_weights(
            "/home/j00492398/test_joey/interface-pred/Src/savedModel/bestModels/RNN_best_model_and_weights.h5",
            by_name=True)
    else:
